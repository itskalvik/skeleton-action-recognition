...
def get_parser():
    ...
    parser = argparse.ArgumentParser(
        description=
        'Graph Convolutional Neural Network for Skeleton-Based Action Recognition'
    )
    parser.add_argument('--model', required=True, help='model used to train')
    ...
    parser.add_argument('--save-freq',
                        type=int,
                        default=10,
                        help='periodicity of saving model weights')
    parser.add_argument(
        '--freeze-graph-until',
        type=int,
        default=80,
        help='adjacency matrices will be trained only after this epoch')
    parser.add_argument(
        '--log-dir',
        default="logs/",
...
def train_step(features, labels, train_adj):
    def step_fn(features, labels, train_adj):
        ...
        trainable_variables = [
            variable for variable in model.trainable_variables
            if not "adjacency_matrix" in variable.name
        ]
...
if __name__ == "__main__":
    ...
    with strategy.scope():
        ...
        train_acc_top_5 = tf.keras.metrics.TopKCategoricalAccuracy(
            name='train_acc_top_5')

    epoch_test_acc = tf.keras.metrics.CategoricalAccuracy(
        name='epoch_test_acc')
    epoch_test_acc_top_5 = tf.keras.metrics.TopKCategoricalAccuracy(
        name='epoch_test_acc_top_5')
    test_acc_top_5 = tf.keras.metrics.TopKCategoricalAccuracy(
        name='test_acc_top_5')
        ...
        with strategy.scope():
            for features, labels in tqdm(train_data):
                train_step(features, labels,
                           True if epoch > freeze_graph_until else False)
                with summary_writer.as_default():
        ...
        if (epoch + 1) % save_freq == 0:
            ...
            cm_image = plot_to_image(
                plot_confusion_matrix(
                    cm, class_names=[str(i) for i in range(num_classes)]))
            with summary_writer.as_default():
    ...
    ckpt_save_path = ckpt_manager.save()
    print('Saving final checkpoint for epoch {} at {}'.format(

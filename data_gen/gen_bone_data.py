        ...
        for set in sets:
            ...
            data = np.load(
                os.path.join(datapath,
                             '{}/{}_data_joint.npy'.format(dataset, set)))
            N, C, T, V, M = data.shape
            fp_sp = open_memmap(os.path.join(
                datapath, '{}/{}_data_bone.npy'.format(dataset, set)),
                                dtype='float32',
                                mode='w+',
                                shape=(N, 3, T, V, M))
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description='NTU-RGB-D Data Joint to Bone Converter.')
